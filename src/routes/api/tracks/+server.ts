...
export async function GET({ locals, url }: RequestEvent) {
	try {
		...
		const tracks = await getTracksByUser(auth.user.id, limit, offset);
		return json(tracks);
	} catch (err) {
...
export async function POST({ request, locals }: RequestEvent) {
	try {
		...
		// LOG: show incoming cookies/header to diagnose why auth fails
		try {
			const cookieHeader = request.headers.get('cookie');
			console.log('[api/tracks] request cookie header:', cookieHeader);
		} catch (e) {
			console.warn('[api/tracks] could not read cookie header', e);
		}
		...
		console.log(`[api/tracks] saving file for user=${auth.user.id} name=${(file as any).name}`);

		// write file to storage
		await fs.mkdir(STORAGE_DIR, { recursive: true });
		const ownerDir = `user-${auth.user.id}`;
		...
		const serverFileName = `${crypto.randomUUID()}-${Date.now()}-${(file as any).name}`;
		...
		const wasVideo = (file as any).type?.startsWith('video/') ?? false;

		console.log(`[api/tracks] file written to disk: ${absPath}`);

		const created = await createTrack(auth.user.id, (file as any).name, relPath, title || undefined, wasVideo);
		console.log(`[api/tracks] track created in DB: id=${created.id} user=${auth.user.id} path=${relPath}`);
